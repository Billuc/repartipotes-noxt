...
export function getExpenses(splitId: string): Expense[] {
  const d = getDb();
  const rows = d.query<ExpenseRow, string>(
    "SELECT * FROM expenses WHERE split_id = ?"
  ).all(splitId);
  return rows.map(rowToExpense);
}

export function createExpense(input: CreateExpenseInput): number {
  const d = getDb();
  const result = d.query<unknown, [string, string, number, string, number, string, string, string, number, string]>(
...
export function updateExpense(input: UpdateExpenseInput): void {
  const d = getDb();
  d.query<unknown, [string, number, string, number, string, string, string, number, string, number]>(
    `UPDATE expenses
...
export function deleteExpense(id: number, splitId: string): void {
  const d = getDb();
  d.query<unknown, [number, string]>(
